...
from utils import get_freq, load_model, Normalize, get_comp_wv_batch, get_LL_batch, get_ll_orig_batch, get_HFmasked_ll_batch, get_ll_orig_inverse_batch, visualize_inputs, get_ll_orig_inverse_batch8, get_HFtop_comp_batch ,get_HFtop_comp_inv_batch, get_HFtop_comp_lev_inv_batch, get_HFtop_comp_lev_batch, get_HFtop_compfrom_HF_batch, get_HFtop_compfrom_HF_inv_batch
import torch
import torchattacks
import numpy as np
...
#         self.fake_A = get_ll_orig_inverse_batch(self.denorm(self.fake_A)*2).float().to(self.device)
#         visualize_inputs(self.real_B, 'real_B')
#         print('real_B: ', torch.min(self.real_B),torch.max(self.real_B))        
...
#         self.real_A = get_ll_orig_inverse_batch(self.denorm(self.real_A)*2).float().to(self.device)
        self.real_A = get_HFtop_comp_lev_inv_batch(self.real_B_orig,self.denorm(self.real_A)*2,keep=0.15,lev=1).float().to(self.device)
#         exit()
        if self.adv==True:
...
#         exit()
    ...
    def eval_teacher(self):
        ...
        with torch.no_grad():
            ...
            if self.adv==True:              
                self.teach_real_pred = self.netT(self.adv_fake_A)
