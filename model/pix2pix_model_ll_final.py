...
from utils import load_model, Normalize, spectral_loss, get_ll_orig_batch, get_HFmasked_ll_batch, get_ll_orig_inverse_batch, get_ll_orig_inverse_batch8, visualize_inputs 
import numpy as np
...
#         self.real_B = input[0].to(self.device)   
#         print(torch.min(self.real_B), torch.max(self.real_B))
#         self.real_A = input[1].to(self.device)    # ll' l1
...
#         asd = self.denorm(self.fake_B)
#         print(torch.min(asd), torch.max(asd))
#         visualize_inputs(self.fake_B[0],'output')
#         visualize_inputs(self.real_A[0],'input')
#         exit()
...
#             self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) * self.opt.lambda_L1
    ...
    def set_toy_imgs(self, dataloader):
        for data, gt_data, lp_data, label in dataloader:
            break
#         self.toy_real_B = get_HFmasked_ll_batch(data,lev=2)[0].unsqueeze(0).float().to(self.device)/4
#         self.toy_real_A  = get_ll_orig_batch(data,lev=2)[0].unsqueeze(0).float().to(self.device)/4
        self.toy_real_B = lp_data[0].to(self.device)
        self.toy_real_A = gt_data[0].to(self.device)
#         print(self.toy_real_B.size())
#         print(self.toy_real_A.size())
#         exit()
#         self.toy_real_A  = get_ll_orig_batch(data,lev=2)[0].unsqueeze(0).float().to(self.device)/4
