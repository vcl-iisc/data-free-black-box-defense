...
from utils import get_freq, cosim_loss, get_comp_wv_batch, Res18PerceptualLoss, TV_loss, mixup_data, mixup_wavelet, spectral_loss, get_LL_batch, visualize_inputs, mixup_wavelet_custom, get_k_compfrom_HF_batch, prog_k, get_LL_batch_mnist, get_HFtop_compfrom_HF_batch_mnist, get_gn_perturbed_img_batch, get_LL_mnist_torch
import numpy as np
from util.NCEAverage import NCEAverage
from util.NCECriterion import NCECriterion
from DA.utils import load_model, Normalize, Trades_adv, MMD_Loss
...
class Pix2PixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.opt.reg == 'cosim_CS':
            self.loss_names = ['G_onlycosim', 'G_cs', 'gen_Acc', 'og_Acc', 'lp_Acc', 'gen_adv_Acc', 'adv_Acc', 'lp_adv_Acc']
        if self.opt.reg == 'cosim_MMD':
            self.loss_names = ['G_onlycosim', 'G_mmd', 'gen_Acc', 'og_Acc', 'lp_Acc', 'gen_adv_Acc', 'adv_Acc', 'lp_adv_Acc']
...
#         self.real_adv = get_gn_perturbed_img_batch(self.real_B, self.opt.gn_stddev)             #real_B :> originalimage 
        self.real_adv = input[1]
#         self.Final_model = nn.Sequential(self.norm, self.netG.module, self.denorm, self.netF,self.netB,self.netC)
#         self.Final_model.eval()
#         if self.opt.adv_samples=='MMD':
#             self.Final_model = nn.Sequential(self.ll_transform, self.norm, self.netG.module, self.denorm, self.netF, self.netB)
#             self.Final_model.eval()    
#         self.attack = torchattacks.PGD(self.Final_model, eps=0.3, alpha=0.01, steps=40)     #train time parameters
#         self.real_adv = Trades_adv(self.real_B,40,0.3,'l_inf',0.01,self.Final_model,self.opt.adv_samples,self.device)   #train parameters
#         self.real_adv = self.attack(self.real_B, self.labels)
#         self.netF.eval()
#         self.netB.eval()
#         self.netC.eval()
#         self.netG.train()
#         self.real_adv = Variable(torch.clamp(self.real_adv, 0.0, 1.0), requires_grad=False)
...
#             self.real_A  = get_HFtop_compfrom_HF_batch_mnist(self.real_B, lev=self.opt.decomposition_level, keep = self.opt.coefficients_percent/100).float().to(self.device)
        elif self.opt.transform_type=='ll':
            self.real_A = get_LL_batch_mnist(self.real_B, level=self.opt.decomposition_level).float().to(self.device)
            self.real_A_adv = get_LL_batch_mnist(self.real_adv, level=self.opt.decomposition_level).float().to(self.device)
#         self.real_A_lev_1 = self.norm(self.real_A_lev_1)
        self.real_A = self.norm(self.real_A)    #LL (original image)     
        self.real_B = self.norm(self.real_B)    # Original Image
        self.real_adv = self.norm(self.real_adv)  #Adv Image   
        self.real_A_adv = self.norm(self.real_A_adv)   # LL(Adv Image)
...
#             self.indexes = input[-1].to(self.device)
    ...
    def forward(self):
        ...
        self.fake_B = self.netG(self.real_A)  # Translated(LL(Original))
        self.fake_B_adv = self.netG(self.real_A_adv)  # Translated(LL(Adv))
...
#             self.loss_G_L1_ll1 = self.criterionL1(self.fake_B_lev_1, self.real_B_lev_1) * 50
        ...
        if self.opt.reg == 'cosim_KL':
            teacher_pred_real_B = self.netC(self.netB(self.netF(self.denorm(self.real_B))))  # Logits (Original Image)
            teacher_pred_fake_B = self.netC(self.netB(self.netF(self.denorm(self.fake_B))))  # Logits (Trans(LL(Orig))
...
#             print(self.loss_G_onlycosim)
#             print(self.opt.kl_weight*self.loss_G_kl)
            self.loss_G = self.loss_G_onlycosim + self.opt.reg_weight*self.loss_G_kl # + self.loss_G_CE
        ...
        if self.opt.reg == 'cosim_CS':
            ...
            teacher_pred_fake_B_adv = self.netC(self.netB(self.netF(self.denorm(self.fake_B_adv))))
            self.loss_G_onlycosim = self.criterioncosim(teacher_pred_fake_B ,teacher_pred_real_B.detach())
#             self.loss_G_ce_clean = self.crossentropy(teacher_pred_fake_B,self.labels)
            self.loss_G_cs = self.opt.reg_weight*self.criterioncosim(teacher_pred_fake_B_adv ,teacher_pred_fake_B.detach())
# #             self.loss_G_ce_adv = self.opt.reg_weight*self.crossentropy(teacher_pred_fake_B_adv,self.labels)
#             print(self.loss_G_ce_clean)
#             print(self.loss_G_ce_adv)
            self.loss_G = self.loss_G_onlycosim + self.loss_G_cs 
        ...
        if self.opt.reg == 'cosim_MMD':
            ...
            teacher_pred_fake_B_adv = self.netC(self.netB(self.netF(self.denorm(self.fake_B_adv))))
            self.loss_G_onlycosim = self.criterioncosim(teacher_pred_fake_B ,teacher_pred_real_B.detach())
            loss_mmd, _, _ = self.dist_criterion(self.netB(self.netF(self.denorm(self.fake_B_adv))) ,self.netB(self.netF(self.denorm(self.fake_B))).detach())
            self.loss_G_mmd = self.opt.reg_weight*loss_mmd
            self.loss_G = self.loss_G_onlycosim + self.loss_G_mmd 
...
#         return x_new
    ...
    def eval_teacher(self):
        ...
        with torch.no_grad():
            ...
            self.teach_fake_B_pred = self.netC(self.netB(self.netF(self.denorm(self.fake_B))))
            ...
            self.teach_real_A_pred = self.netC(self.netB(self.netF(self.denorm(self.real_A))))
            ...
            ## Accuracy on clean
            self.teach_real_B_pred = self.netC(self.netB(self.netF(self.denorm(self.real_B))))
            ...
            self.teach_fake_B_adv_pred = self.netC(self.netB(self.netF(self.denorm(self.fake_B_adv))))
            ...
            # Accuracy on Low Pass
            self.teach_real_A_adv_pred = self.netC(self.netB(self.netF(self.denorm(self.real_A_adv))))
            ...
            ## Accuracy on clean
            self.teach_real_adv_pred = self.netC(self.netB(self.netF(self.denorm(self.real_adv))))
