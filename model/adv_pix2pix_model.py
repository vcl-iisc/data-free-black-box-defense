...
from utils import load_model, get_freq, Normalize, cosim_loss, get_comp_wv_batch, Res18PerceptualLoss, TV_loss, mixup_data, mixup_wavelet, spectral_loss, get_LL_batch, visualize_inputs, mixup_wavelet_custom, get_k_compfrom_HF_batch, prog_k, get_gn_perturbed_img_batch
import numpy as np
...
#         self.real_B_adv = input[1]                 #real_B :> originalimage 
...
#         self.real_B_gt = torch.cat((self.real_B, self.real_B), 0)
        ...
        self.real_B = torch.cat([input[0], input[0]]).to(self.device, dtype=torch.float)     
#         self.real_A = torch.cat([input[0], input[1]]).to(self.device, dtype=torch.float)
        self.real_A = get_gn_perturbed_img_batch(input[0]).float().to(self.device)
...
#         self.labels = torch.cat([input[2], input[3]]).to(self.device)
